...
<form action="/subdivision/{{$subdivision->id}}/preparation_receipt" class="well" id="PreparationReceiptCreateForm" method="post" accept-charset="utf-8">
    {{ csrf_field() }}
    {{ method_field('POST') }}
    ...
    <div class="form-group required">
        ...
        <input name="date" type="date" class="form-control" id="PreparationReceiptDate" value="{{ old('date') }}">
    </div>
    <fieldset>
		<legend>Сведения о документе</legend>
		<div class="form-group">
            <label for="PreparationReceiptDocDate">Дата</label>
            <input name="doc_date" type="date" class="form-control" id="PreparationReceiptDocDate" value="{{ old('doc_date') }}">
        </div>
        <div class="form-group">
            <label for="PreparationReceiptDocNum">Номер</label>
            <input name="doc_num" class="form-control" maxlength="250" type="text" id="PreparationReceiptAddress" value="{{ old('doc_num') }}">
        </div>
        <div class="form-group">
            ...
            <select name="basic_document_id" class="form-control" id="PreparationReceiptBasicDocumentId">
                ...
                @foreach ($basicDocuments as $id => $basicDocument)
                    <option value="{{$id}}"{{ old('basic_document_id') == $id ? 'selected' : '' }}>{{ $basicDocument }}</option>
                @endforeach
            </select>
        </div>
    </fieldset>
    <fieldset>
        ...
        <div class="form-group">
            ...
            <select name="preparation_id" id="PreparationReceiptPreparationId" class="form-control">
                ...
                @foreach ($preparations as $id => $preparation)
                    <option value="{{$id}}" {{ old('preparation_id') == $id ? 'selected' : '' }}>{{ $preparation }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            <label for="PreparationReceiptSeries">Серия</label>
            <input name="series" class="form-control" maxlength="45" type="text" id="PreparationReceiptSeries" value="{{ old('series') }}">
        </div>
        <div class="form-group">
            <label for="PreparationReceiptContainerDoses">Количество доз (мл, г, л, кг) во флаконе (в комплекте флаконов, единице тары)</label>
            <input name="container_doses" class="form-control" type="number" step="1" id="PreparationReceiptContainerDoses" value="{{ old('container_doses') }}">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <select name="purchase_type" class="form-control" id="PreparationReceiptPurchaseType">
                <option value="федеральный бюджет" {{ old('purchase_type') == 'федеральный бюджет' ? 'selected' : '' }}>федеральный бюджет</option>
                <option value="СГЗ" {{ old('purchase_type') == 'СГЗ' ? 'selected' : '' }}>СГЗ</option>
                <option value="СИЦ" {{ old('purchase_type') == 'СИЦ' ? 'selected' : '' }}>СИЦ</option>
                <option value="ПД" {{ old('purchase_type') == 'ПД' ? 'selected' : '' }}>ПД</option>
            </select>
        </div>
        <div class="form-group">
            <label for="PreparationReceiptUnitPrice">Стоимость флакона (к-та, единицы тары) (руб.)</label>
            <input name="unit_price" class="form-control" step="0.1" type="number" id="PreparationReceiptUnitPrice" value="{{ old('unit_price') }}">
        </div>
        <div class="form-group">
            <label for="PreparationReceiptComment">Примечание</label>
            <input name="comment" class="form-control" maxlength="255" type="text" id="PreparationReceiptComment" value="{{ old('comment') }}">
        </div>
    </fieldset>
    <div class="form-group">
